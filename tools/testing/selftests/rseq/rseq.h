...
static inline bool rseq_vm_numa_vcpu_id_available(void)
{
	return (int) rseq_feature_size >= rseq_offsetofend(struct rseq_abi, vm_numa_vcpu_id);
}

static inline uint32_t rseq_current_vm_numa_vcpu_id(void)
{
	return RSEQ_ACCESS_ONCE(rseq_get_abi()->vm_numa_vcpu_id);
}
