...
void percpu_counter_tree_add(struct percpu_counter_tree *counter, int inc)
{
	 ...
	 * Guard against preemption rather than migration because this
	 * is a short critical section, a fast-path, and preemption
	 * guard generates more efficient code than migration guard.
	 */
	guard(preempt)();
